...
CI publishes the CLI and MCP executables and runs `packaging/test-mcp-stdio.ps1`, which verifies the initialize handshake and expected tool list without starting WPF. CD attaches the CLI ZIP/checksum plus the MCPB, checksum, and registry metadata to the versioned GitHub Release. Both standalone hosts remain Windows-only and require an interactive desktop session for capture and recording.
